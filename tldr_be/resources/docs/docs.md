...
The `home-routes` handler in the `tldr-be.routes.home` namespace
...
The `home-page` function will in turn call the `tldr-be.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `tldr-be.handler` namespace:
...
Request middleware functions are located under the `tldr_be.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `tldr-be.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `tldr-be.db.core` in some other namespace.
